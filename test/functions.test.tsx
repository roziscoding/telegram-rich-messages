...
test("JSX and functional syntax create the same canonical Telegram value", () => {
  ...
  const functional = rm.richMessage(
    ...
    rm.table(
      ...
      rm.tableRow(rm.tableCell({ header: true, align: "right" }, rm.italic("98.4"))),
    ),
  );
...
test("functional builders enforce hierarchy at runtime for JavaScript and cast callers", () => {
  expect(() => rm.table(rm.paragraph("bad") as never)).toThrow("table() only accepts <table-row>");
...
test("functional rich-text builders cover every entity", () => {
  ...
  const message = rm.richMessage(rm.paragraph(
    ...
    rm.email({ address: "a@example.com" }, "mail"), rm.phone({ number: "+1555" }, "call"),
    rm.bankCard({ number: "4242" }, "card"), rm.mention({ username: "telegram" }, "@telegram"),
    rm.hashtag({ value: "bots" }, "#bots"), rm.cashtag({ value: "TON" }, "$TON"),
    rm.botCommand({ command: "start" }, "/start"), rm.textAnchor({ name: "intro" }),
    rm.anchorLink({ name: "intro" }, "back"), rm.reference({ name: "note" }, "note body"),
...
test("functional block builders cover every block variant", () => {
  ...
  const output = rm.richMessage(
    ...
    rm.list(rm.listItem({ checkbox: true, checked: true }, rm.paragraph("item"))),
    rm.blockQuote({ credit: "Ada" }, rm.paragraph("quote")), rm.pullQuote({ credit: "Roz" }, "pull"),
    rm.collage({ caption: "Gallery" }, rm.photo({ media: photo })),
    rm.slideshow({ caption: "Slides" }, rm.video({ media: video })),
    rm.table({ bordered: true }, rm.tableRow(rm.tableCell({ header: true }, "Cell"))),
