...
function _typeSafetyAssertions(): void {
    ...
    const permissiveMessage: InputRichMessage = {
        blocks: [],
        html: "<p>Hello</p>",
    };
    ...
    const functionalRow: TableRowValue = rm.tableRow(
        rm.tableCell(rm.bold("typed")),
    );
    ...
    expectType<InputRichBlockPhoto>(
        rm.photo({ media: { type: "photo", media: "photo-id" } }),
    );
    expectType<InputRichMessage>(
        rm.richMessage(rm.paragraph("canonical message")),
    );
